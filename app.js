...
*/
window.GA_APP_BUILD = '2026-08-09-v185-dwarf-forge-casino';
...
(function normalizeAthleteIdentityV72() {
  ...
  */
  ...
  const isMagicarpe = hasSignal(/^(?:clara-)?(?:magicarpe|magicapre)(?:-|$)/);
...
window.GA_APP_BUILD = window.GA_APP_BUILD || '2026-08-06-v149-raid-ultra-fun';
  ...
  const ACTIVITY_DISPLAY = {
    ...
    magicapre: { name: 'Magicarpe', emoji: '🐟' },
...
(function () {
  ...
  let progress = null;
  let adventureProgressErrorV160 = '';
  let adventureProgressLoadedV160 = false;
  let inventory = [];
  let monsterCatalog = [];
...
function currentEquipmentPower() {
  return Math.max(0, n(statSnapshot()?.gear?.power));
}

function requiredEquipmentLevelForDifficulty(value = currentAdventureDifficulty()) {
  const d = Math.min(MAX_ADVENTURE_DIFFICULTY, Math.max(1, Math.floor(n(value, 1))));
...
function monsterEncounterOdds() {
  ...
  const weights = {
    // Pool de base V163 — probabilités sans Chance / Chasseur :
    // Simple 33,889 %, Commun 30 %, Peu commun 20 %, Rare 10 %,
    // Épique 5 %, Légendaire 1 %, Mythique 0,1 %,
    // Ultra mythique 0,01 %, Abyssal 0,001 %.
    normal: 33.889,
    common: 30,
    uncommon: 20 * uncommonMultiplier,
    rare: 10 * rarePlusMultiplier * hunterMultiplier,
    epic: 5 * rarePlusMultiplier * hunterMultiplier,
    legendary: 1 * rarePlusMultiplier * hunterMultiplier,
    mythic: 0.1 * rarePlusMultiplier * hunterMultiplier,
    ultra_mythic: 0.01 * rarePlusMultiplier * hunterMultiplier,
    abyssal: 0.001 * rarePlusMultiplier * hunterMultiplier
  };
...
function monsterPalierXpMultiplier(
  ...
  // V166 : écart EXACT de paliers.
  // Exemple max 173 :
  // 124 => écart 49 => XP encore actif.
  // 123 => écart 50 => 0 XP.
  const gap = Math.max(0, maximum - fought);

  if (gap >= 50) return 0;
  if (gap >= 40) return 0.45;
  if (gap >= 30) return 0.60;
  if (gap >= 20) return 0.75;
  if (gap >= 10) return 0.90;
  return 1;
}
...
function monsterXpMonitorLabel(
  ...
  if (!(palierMultiplier > 0)) {
    ...
    const fought = Math.max(1, Math.floor(n(difficulty, 1)));
    const gap = Math.max(0, maximum - fought);
    return `0 XP · écart ${gap} paliers avec ton maximum ${maximum} (seuil : 50)`;
  }
...
function monsterEncounterOddsHtml() {
  ...
  <div class="encounter-odds-note"><strong>Pool de base :</strong> 33,889 % Simple, 30 % Commun, 20 % Peu commun, 10 % Rare, 5 % Épique, 1 % Légendaire, 0,1 % Mythique, 0,01 % Ultra mythique et 0,001 % Abyssal. La Chance augmente ensuite Peu commun et Rare+, et Chasseur renforce encore Rare+.<br><strong>XP de farm :</strong> écart avec ton palier max 0–9 = 100 %, 10–19 = 90 %, 20–29 = 75 %, 30–39 = 60 %, 40–49 = 45 %, <strong>50+ = 0 XP</strong>. Le calcul utilise l’écart exact : max 173 → 124 donne encore de l’XP, 123 donne 0 XP.<br><strong>Résistance d’équipement :</strong> tous les monstres sans exception, Abyssaux compris, appliquent le malus de sous-équipement. Seules les pièces Ultra mythiques et Abyssales sont immunisées au malus de niveau pour leur propre emplacement.</div>`;
...
async function armCombatServerTimer(session) {
  ...
  function difficultySelectorHtml() {
    ...
    return `<div class="world-picker">
      ...
      ${(() => {
        ...
        <div class="world-picker-note">${progressionGearPenalty(selected) < 1 ? `<strong>Résistance du monstre :</strong> dégâts ×${fr(progressionGearPenalty(selected),3)} à cause du sous-équipement. Cette résistance existe pour toutes les raretés de monstres, Abyssal compris. Les pièces Ultra mythiques/Abyssales sont immunisées uniquement pour leur propre emplacement.` : selected < unlocked ? '<strong>Équipement couvert :</strong> aucune résistance de sous-équipement. Les pièces Ultra mythiques/Abyssales restent immunisées pour leur propre emplacement.' : '<strong>Équipement couvert :</strong> aucune résistance de sous-équipement. Les pièces Ultra mythiques/Abyssales restent immunisées pour leur propre emplacement.'} Les monstres sont tirés aléatoirement dans tout le bestiaire.</div>`;
      })()}
    </div>`;
  }
  ...
  function classProfileHtml() {
    ...
    return `<div class="xp-section">
      ...
      <button type="button" id="rpgDamageTrial" class="damage-trial-launch" ${canPlay ? '' : 'disabled'}>${canPlay ? '🗿 Monolithe · Affronter Roi Noeil pendant 30 s' : 'Test en lecture seule'}</button>
      <div class="damage-trial-note"><strong>Roi Noeil n’a aucune résistance de palier ni de niveau d’équipement.</strong> Ton build frappe toujours à 100 % de ses dégâts. Record personnel : <b>${fr(progress?.best_damage_trial,0)}</b> dégâts / 30 s · ${n(progress?.damage_trial_attempts)} tentative${n(progress?.damage_trial_attempts)===1?'':'s'}.</div>
    </div>`;
  }
...
function collectionHtml() {
  ...
  function leaderboardSortValue(row, mode = leaderboardSort) {
    ...
    if (mode === 'monolith') return leaderboardBigInt(row?.monolith_best_damage_30s);
  ...
  function leaderboardPrimaryValue(row) {
    ...
    if (leaderboardSort === 'monolith') return `${formatHugeNumberV155(row?.monolith_best_damage_30s)} dégâts / 30 s`;
    ...
    const table = rows.map((row, index) => {
      ...
      return `<article class="rpg-leaderboard-row ${isMe ? 'me' : ''}">
        ...
        <div class="rpg-leaderboard-details">
          ...
          <span class="rpg-leaderboard-detail">Palier max : <b>${n(row.adventure_difficulty,1)}</b> · Boss : <b>${n(row.boss_wins)}</b> · meilleur coup : <b>${formatHugeNumberV155(row.best_combat_damage)}</b><br>🗿 Roi Noeil : <b>${formatHugeNumberV155(row.monolith_best_damage_30s)} dégâts / 30 s</b> · ${n(row.monolith_attempts)} essai${n(row.monolith_attempts)===1?'':'s'}</span>
        </div>
      </article>`;
    }).join('');
  ...
  async function loadRpgLeaderboardV155(force = false) {
    ...
    const [rpgResponse, monolithResponse] = await Promise.all([
      CoachingCloud.client.rpc('get_rpg_leaderboard_v155'),
      CoachingCloud.client.rpc('get_rpg_monolith_leaderboard_v168')
    ]);
    leaderboardBusy = false;

    if (rpgResponse?.error) {
      leaderboardError = `Leaderboard indisponible : ${rpgResponse.error.message}`;
    ...
    const baseRows = Array.isArray(rpgResponse?.data)
      ? rpgResponse.data
      : rpgResponse?.data ? [rpgResponse.data] : [];
    const monolithRows = Array.isArray(monolithResponse?.data)
      ? monolithResponse.data
      : monolithResponse?.data ? [monolithResponse.data] : [];

    const monolithBySlug = new Map(
      monolithRows.map(row => [String(row?.athlete_slug || ''), row])
    );

    leaderboardRows = baseRows.map(row => {
      const mono = monolithBySlug.get(String(row?.athlete_slug || '')) || {};
      return {
        ...row,
        monolith_best_damage_30s: mono.best_damage_30s ?? 0,
        monolith_attempts: mono.attempts ?? 0,
        monolith_last_attempt_at: mono.last_attempt_at ?? null
      };
    });

    for (const mono of monolithRows) {
      if (leaderboardRows.some(row => row.athlete_slug === mono.athlete_slug)) continue;
      leaderboardRows.push({
        athlete_slug: mono.athlete_slug,
        display_name: mono.display_name,
        rpg_class: mono.rpg_class,
        level: mono.level ?? 1,
        adventure_difficulty: mono.adventure_difficulty ?? 1,
        total_damage: '0',
    ...
    if (monolithResponse?.error) {
      leaderboardError = `Classement Monolithe indisponible : ${monolithResponse.error.message}`;
      console.warn(leaderboardError);
    }
  ...
  async function loadAdventureProgressV160({ quiet = false } = {}) {
    if (!window.CoachingCloud?.client || !CoachingCloud.session?.user) {
      return false;
    }

    /*
       V160 — lecture directe et minimale.

       On ne dépend plus d'une RPC, d'une longue liste de colonnes ni d'un
       fallback. Les trois valeurs sont lues directement dans athlete_progress.
    */
    const { data, error } = await CoachingCloud.client
      .from('athlete_progress')
      .select(
        'athlete_slug,adventure_difficulty,kills_toward_boss,boss_wins'
      )
      .eq('athlete_slug', cfg.slug)
      .maybeSingle();

    if (error) {
      adventureProgressLoadedV160 = false;
      adventureProgressErrorV160 =
        `${error.code || 'API'} · ${error.message}`;

      console.warn('Palier aventure V160 indisponible :', error);

      if (!quiet) {
        CoachingCloud.toast(
          `Lecture du palier impossible : ${error.message}`,
          true
        );
      }
    ...
    if (!data) {
      adventureProgressLoadedV160 = false;
      adventureProgressErrorV160 =
        `Aucune ligne athlete_progress pour ${cfg.slug}`;

      if (!quiet) {
        CoachingCloud.toast(
          `Aucune progression RPG trouvée pour ${cfg.slug}.`,
          true
        );
      }
    ...
    if (
      ...
      adventureProgressLoadedV160 = false;
      adventureProgressErrorV160 =
        `Mauvais slug reçu : ${data.athlete_slug || 'vide'}`;
      return false;
    }

    progress = {
      ...(progress || {}),
      athlete_slug: data.athlete_slug,
      adventure_difficulty: Math.min(
        MAX_ADVENTURE_DIFFICULTY,
        Math.max(
          1,
          Math.floor(n(data.adventure_difficulty, 1))
        )
      ),
      kills_toward_boss: Math.max(
        ...
        Math.min(
          50,
          Math.floor(n(data.kills_toward_boss, 0))
        )
      ),
      boss_wins: Math.max(
        0,
        Math.floor(n(data.boss_wins, 0))
      )
    };

    adventureProgressLoadedV160 = true;
    adventureProgressErrorV160 = '';
    return true;
  }
  ...
  async function loadProgress() {
    ...
    */
    ...
    if (result.error) {
      ...
      const adventureRecovered = await loadAdventureProgressV160({ quiet: true });
      if (!adventureRecovered) {
        adventureProgressErrorV160 = `${result.error.code || 'API'} · ${result.error.message}`;
        CoachingCloud.toast(
    ...
    await loadAdventureProgressV160({ quiet: true });
    ...
    const selectorMigrationKey =
      `rpg_adventure_selector_v160_${cfg.slug}`;

    if (
      unlocked > 1
      && localSelected === 1
      && !localStorage.getItem(selectorMigrationKey)
    ) {
      selectedDifficulty = unlocked;
    } else {
      selectedDifficulty = Math.min(unlocked, localSelected);
    }

    localStorage.setItem(selectorMigrationKey, '1');
    localStorage.setItem(
  ...
  function ensureDamageTrialOverlay() {
    ...
    overlay.innerHTML = `<div class="trial-arena">
      <div id="trialFightView">
        <div class="trial-map-title">Monolithe · Test de dégâts 30 secondes</div>
        <div class="trial-map-subtitle">👑👁️ Roi Noeil · résistance de palier : ×1,000</div>
        <div class="trial-clock" id="trialClock">30,0</div>
        <div class="reaction-hint" id="trialReactionHint">Observe l’écran…</div>
        <div class="trial-stage" id="trialStage"><div class="trial-portal"></div><div class="trial-map-floor"></div><button type="button" class="trial-dummy" id="trialDummy" aria-label="Roi Noeil">👑👁️</button><button type="button" class="reaction-target hidden" id="trialReactionTarget"></button></div>
  ...
  async function startDamageTrial() {
    ...
    if (button) button.disabled = true;
    const { data, error } = await CoachingCloud.client.rpc('start_rpg_monolith_v168', { p_athlete_slug: cfg.slug });
    if (button) button.disabled = false;
    if (error) {
  ...
  async function finishDamageTrial() {
    ...
    let response = await damageTrialRpcTimeout(
      CoachingCloud.client.rpc('finish_rpg_monolith_v168', {
        p_trial_id: trial.id,
        p_actions: combatActionsPayloadV64(trial)
      })
    );
    ...
    if (resultText) {
      resultText.innerHTML = `Roi Noeil a encaissé <strong>${fr(score,0)} dégâts en 30 secondes</strong> avec <strong>${n(result?.successful_actions)}</strong> actions réussies.<br>Précision <strong>${fr(result?.accuracy_pct,0)} %</strong> · parfaits <strong>${n(result?.perfect_actions)}</strong> · combo max <strong>×${n(result?.max_combo)}</strong> · perfect streak max <strong>${n(result?.max_perfect_streak)}</strong>.<br>${isRecord ? '<strong>🏆 Nouveau record personnel Monolithe !</strong><br>' : ''}Meilleur score : <strong>${fr(result?.best_damage_trial,0)} dégâts / 30 s</strong>.<br>Critiques de Chance : <strong>${n(result?.crit_count)}</strong>.<br><strong>Résistance de palier : ×1,000.</strong><br><br>Aucun XP, gold ou objet n’est distribué sur le Monolithe.`;
    }
    progress = {
      ...progress,
  ...
  async function finishCombat(reason = 'automatic') {
    ...
    const rpcPromise = CoachingCloud.client.rpc(
      combat.isBoss ? 'finish_rpg_boss_codex_xp_v35' : 'finish_rpg_combat_v166',
      {
  ...
  const GL_V171_FEMALE_SLUGS = new Set([
    'celia','janel','kaoutar','lou','lucine',
    'metaknight','sarah','saya','serena','magicarpe','marvin'
  ]);

  const GL_V171_PRIMARY_FILES = {
    alexandre:'Alexandre.html',
    benoit:'Benoit.html',
    celia:'Celia.html',
    charles:'Charles.html',
    clemosaurus:'Clemosaurus.html',
    dorian:'Dorian.html',
    duane:'Duane.html',
    flop:'Flop.html',
    gibertini:'gibertini.html',
    guillaume:'Anzalone.html',
    hugo:'Hugo.html',
    janel:'Janel.html',
    jolan:'Jolan.html',
    jonathan:'Jonathan.html',
    kaoutar:'Kaoutar.html',
    killian:'Killian.html',
    lou:'Lou.html',
    louis:'Louis (2).html',
    lucine:'Lucine.html',
    magicarpe:'Magicarpe.html',
    malo:'Malo.html',
    matthieu:'Matthieu.html',
    maxence:'Maxence.html',
    metaknight:'Metaknight.html',
    noe:'Noe.html',
    sarah:'Sarah.html',
    saya:'Saya.html',
    serena:'Serena.html',
    tom:'TomDeneuville.html',
    yann:'Yannick.html'
  };

  function glV171Number(value) {
    const parsed = Number(String(value ?? '').replace(',','.'));
    return Number.isFinite(parsed) ? parsed : null;
  }

  function glV171ExtractExact(label,text) {
    const escaped=String(label).replace(/[.*+?^${}()|[\]\\]/g,'\\$&');
    const match=String(text||'').match(
      new RegExp(`\\b${escaped}\\s*:\\s*~?(-?\\d+(?:[.,]\\d+)?)\\s*(?:kg)?`,'i')
    );
    return match?glV171Number(match[1]):null;
  }

  function glV171BodyWeight(text) {
    const configured=glV171Number(cfg?.bodyWeight);
    if(configured>0)return configured;

    for(const pattern of [
      /Poids utilisé\s*:\s*~?(\d+(?:[.,]\d+)?)/i,
      /Poids de référence\s*:\s*~?(\d+(?:[.,]\d+)?)/i,
      /Poids\s*:\s*~?(\d+(?:[.,]\d+)?)/i,
      /PDC\s*:\s*~?(\d+(?:[.,]\d+)?)/i
    ]){
      const match=String(text||'').match(pattern);
      const value=match?glV171Number(match[1]):null;
      if(value>0)return value;
    }
    return null;
  }

  function glV171Sex(text) {
    if(/Genre\s*:\s*Femme/i.test(text||''))return 'F';
    if(/Genre\s*:\s*Homme/i.test(text||''))return 'M';
    return GL_V171_FEMALE_SLUGS.has(String(cfg?.slug||'').toLowerCase())?'F':'M';
  }

  async function syncCurrentProgramGlV171() {
    if(!window.CoachingCloud?.client||!CoachingCloud.session?.user)return null;
    if(!cfg?.slug||!cfg?.programKey)return null;

    const slug=String(cfg.slug).toLowerCase();
    const expectedFile=GL_V171_PRIMARY_FILES[slug];

    if(expectedFile){
      const currentFile=decodeURIComponent(
        String(location.pathname||'').split('/').pop()||''
      );
      if(currentFile&&currentFile.toLowerCase()!==expectedFile.toLowerCase()){
        return null;
      }
    }

    const text=String(document.body?.innerText||'').replace(/\s+/g,' ');
    const squat=glV171ExtractExact('Squat',text);
    const bench=glV171ExtractExact('Bench',text);
    const deadlift=glV171ExtractExact('Deadlift',text);
    const bodyweight=glV171BodyWeight(text);
    const sex=glV171Sex(text);

    if(!(squat>0&&bench>0&&deadlift>0&&bodyweight>0))return null;

    const {data,error}=await CoachingCloud.client.rpc(
      'sync_athlete_program_gl_v171',
      {
        p_athlete_slug:slug,
        p_program_key:cfg.programKey,
        p_squat_max:squat,
        p_bench_max:bench,
        p_deadlift_max:deadlift,
    ...
    if(error){
      if(!/access|accès|permission|connecté/i.test(error.message||'')){
        console.warn('Synchronisation GL V171 impossible :',error.message);
      }
      return null;
    }

    const row=Array.isArray(data)?data[0]:data;
    if(row?.gl_points!=null){
      progress={...(progress||{}),gl_points:n(row.gl_points)};
      render();
    }
    return row||null;
  }
