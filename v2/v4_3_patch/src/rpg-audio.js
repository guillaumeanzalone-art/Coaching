...
const CRITICAL_SFX_FILE = 'critical-ragnarok.mp3'
const CRITICAL_SFX_POOL_SIZE = 4
let criticalPool = []
let criticalPoolIndex = 0
let criticalUnavailable = false
let criticalFallbackContext = null
let criticalRestoreTimer = null
...
function duckMusicForImpact(
  ...
  if (criticalRestoreTimer) {
    clearTimeout(
      criticalRestoreTimer
    )
  }
...
function criticalFallback() {
  ...
  try {
    ...
    // Le visuel critique reste actif
    // même sans audio disponible.
  }
}

export function preloadRpgCriticalSfx() {
  const pool =
    ensureCriticalPool()
...
export function playRpgCriticalSfx() {
  if (!rpgSfxAllowed()) {
    return false
  }
...
function controlsHtml() {
  ...
  return `
    ...
    >
      ...
      <button
        type="button"
        class="rpg-audio-test-v2"
        data-rpg-audio-test-critical
      >
        💥 Tester le critique
        <small>
          Utilise automatiquement le son Ragnarok s'il est présent.
        </small>
      </button>
