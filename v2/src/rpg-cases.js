...
async function playOpeningAnimation(
  ...
  overlay.innerHTML = `
    <div class="rpg-case-opening-v2">
      <div data-rpg-case-animation>
          ...
          >
            ${rollItems
              .map(item => {
                ...
                return `
                  <div
                    class="rpg-case-roll-item-v2 rarity-${item.item_rarity}"
                  >
                    <span>
                      ${
                        SLOT_ICONS[
        ...
        >
          Aucun bouton Skip · le résultat vient déjà de Supabase.
        </div>
      </div>
  ...
  if (status) {
    ...
    status.textContent =
      `${rarity.label} · ${showcase.item_name}`
  }
  ...
  if (results) {
    ...
    results.innerHTML = `
      ...
      <div class="rpg-case-result-grid-v2">
        ${aggregated
          .map(item => {
            const rarity =
              RARITIES[
            ...
            return `
              <div
                class="rpg-case-result-v2 rarity-${item.item_rarity}"
              >
                <span class="rpg-case-result-icon-v2">
                  ${
                    SLOT_ICONS[
                      item.item_slot
                    ] ||
