...
const RARITY_ORDER = { normal:0, common:1, uncommon:2, rare:3, epic:4, legendary:5, mythic:6, ultra_mythic:7, abyssal:8 }

const equipmentUiState = { sort:'rarity', rarity:'all' }

export function updateRpgEquipmentUi(element) {
  if (!element) return false

  if (element.matches('[data-rpg-equipment-sort]')) {
    equipmentUiState.sort =
      element.value === 'level'
        ? 'level'
        : 'rarity'
    return true
  ...
  if (element.matches('[data-rpg-equipment-rarity]')) {
    equipmentUiState.rarity =
      element.value || 'all'
    return true
  }

  if (element.matches('[data-rpg-equipment-sort-value]')) {
    equipmentUiState.sort =
      element.dataset.rpgEquipmentSortValue === 'level'
        ? 'level'
        : 'rarity'
    return true
  }

  if (element.matches('[data-rpg-equipment-rarity-value]')) {
    equipmentUiState.rarity =
      element.dataset.rpgEquipmentRarityValue || 'all'
    return true
  }
...
function comparisonHtml(item, inventory, progress) {
  if (
    !item ||
    item.equipped ||
    !RPG_SLOT_DEFS[item.slot]
  ) {
    return ''
  }
  ...
  const rows = [
    {
      label: 'Dégâts',
      next: candidateStats.damage,
      current: equippedStats.damage,
      suffix: ' %',
      multiplier: 1,
    },
    {
      label: 'Puissance',
      next: candidateStats.power,
      current: equippedStats.power,
      suffix: '',
      multiplier: statMultiplier,
    },
    {
      label: 'Chance',
      next: candidateStats.mastery,
      current: equippedStats.mastery,
      suffix: '',
      multiplier: statMultiplier,
    },
    {
      label: 'Fortune',
      next: candidateStats.fortune,
      current: equippedStats.fortune,
      suffix: '',
      multiplier: statMultiplier,
    },
  ]
  ...
  const header =
    equipped
      ? `
        <div class="rpg-eq-compare-head">
          <span>Comparé à l'objet équipé</span>
          <b>${esc(equipped.item_name)}</b>
        </div>
      `
      : `
        <div class="rpg-eq-compare-head">
          <span>Comparaison</span>
          <b>Emplacement vide</b>
        </div>
      `

  return `
    <div class="rpg-eq-compare">
      ${header}

      <div class="rpg-eq-compare-table">
        <div class="rpg-eq-compare-row head">
          <span>Stat</span>
          <span>Équipé</span>
          <span>Nouveau</span>
          <span>Écart</span>
        ...
        ${rows
          .map(
            row => {
              ...
              return `
                <div class="rpg-eq-compare-row ${cls}">
                  <b>${esc(row.label)}</b>
                  <span>${fr(current, 2)}${row.suffix}</span>
                  <strong>${fr(next, 2)}${row.suffix}</strong>
                  <em>${sign}${fr(diff, 2)}${row.suffix}</em>
                </div>
              `
            }
          )
      ...
      ${
        affinity === item.slot
          ? `
            <small class="rpg-eq-affinity-note">
              ✨ Affinité de classe ×1,25 incluse dans Puissance, Chance et Fortune.
            </small>
          `
          : ''
      }
...
function inventoryHtml(progress, inventory, canEdit) {
  ...
  const sorted = [...filtered].sort((a,b) => {
    if (equipmentUiState.sort === 'level') return n(b.item_level)-n(a.item_level) || (RARITY_ORDER[b.rarity]||0)-(RARITY_ORDER[a.rarity]||0)
  ...
  return `
    <div class="rpg-eq-bulk">
      <button type="button" data-rpg-deposit-all ${!canEdit || !depositable ? 'disabled' : ''}>📚 Tout déposer<small>${depositable} piles éligibles</small></button>
      <button type="button" data-rpg-sell-all ${!canEdit || !sellable ? 'disabled' : ''}>🪙 Tout vendre<small>${sellable} exemplaires</small></button>
      <p>🔒 Les objets verrouillés et équipés sont protégés des actions globales${lockedQty ? ` · ${lockedQty} protégé${lockedQty>1?'s':''}` : ''}.</p>
    </div>
    <div class="rpg-eq-toolbar">
      <div class="rpg-eq-sort-block">
        <span class="rpg-eq-toolbar-label">Trier</span>

        <div class="rpg-eq-segmented">
          <button
            type="button"
            data-rpg-equipment-sort-value="rarity"
            class="${equipmentUiState.sort === 'rarity' ? 'active' : ''}"
          >
            ✨ Rareté
          </button>
          ...
          <button
            type="button"
            data-rpg-equipment-sort-value="level"
            class="${equipmentUiState.sort === 'level' ? 'active' : ''}"
          >
            ⬆ Niveau
          </button>
        </div>
      ...
      <div class="rpg-eq-rarity-filter">
        <span class="rpg-eq-toolbar-label">Afficher</span>

        <div class="rpg-eq-rarity-pills">
          <button
            type="button"
            data-rpg-equipment-rarity-value="all"
            class="${equipmentUiState.rarity === 'all' ? 'active' : ''}"
          >
            Tous
          </button>

          ${Object.entries(RPG_RARITY_DEFS)
            .map(
              ([key, def]) => `
                <button
                  type="button"
                  data-rpg-equipment-rarity-value="${key}"
                  class="rarity-${key} ${equipmentUiState.rarity === key ? 'active' : ''}"
                >
    ...
    <div class="rpg-eq-inventory">
      ...
      }).join('') || '<div class="rpg-eq-empty">Aucun objet pour ce filtre.</div>'}
    </div>`
}
...
export async function handleRpgEquipmentAction({
  ...
  const itemToLock = element.dataset.rpgLock
  if (itemToLock) {
    element.disabled = true
